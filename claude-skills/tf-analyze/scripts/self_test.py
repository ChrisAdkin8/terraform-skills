...
def main() -> int:
    ...
    for fixture in fixtures:
        ...
        if not expected and not zero_expected:
            ...
            continue
        # For guards-against fixtures, run the full catalogue so we can
        # verify the guarded IDs stay silent even under the full ruleset.
        guards = _parse_guards(fixture / "main.tf") if zero_expected else set()
        actual = run_detect(fixture, name, all_rules=bool(guards))
        if zero_expected:
            if guards:
