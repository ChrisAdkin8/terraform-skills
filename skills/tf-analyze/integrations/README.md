...
**Install:** copy `pre-commit-hook.yaml` into your `.pre-commit-config.yaml` (replace the default `repos:` list, or merge the `hooks:` entry into an existing `repo: local` block).
